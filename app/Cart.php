<?php 
    ...
    class Cart 
    {
        ...
        public function add($data,$quantity=1){
            ...
            if(isset($this->items[$data[0]['product_id']])){    
                $this->items[$data[0]['product_id']]['quantity'] += 1;
            }
            else{
                $this->items[$data[0]['product_id']] = $data;
                $this->items[$data[0]['product_id']]['quantity'] = $quantity;
            }
        ...
        public function total(){
            ...
            if (count($this->items)){
                foreach($this->items as $item){
                    $tong = $tong+($item['quantity']*$item[0]['sale_off']);   
                }
            }
                return $tong;
        }
